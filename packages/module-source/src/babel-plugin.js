 ...
 * ```
 * @param {NodePath} path
 * @param {any} pattern TODO: type this
 */
...
/**
 * Creates paired analyze and transform visitor passes for ESM module source code.
 *
  ...
  const rewriteModules = pass => {
    ...
    const moduleVisitor = (doAnalyze, doTransform) => ({
          ...
          for (const spec of specs) {
            ...
            switch (spec.type) {
              ...
              case 'ImportSpecifier':
                // @ts-expect-error - type only on ImportSpecifier
      ...
      ClassDeclaration(path) {
        ...
        // @ts-expect-error - wants Identifier
      ...
      ExportNamedDeclaration(path) {
        ...
        if (doAnalyze) {
          ...
          if (decl) {
            // @ts-expect-error - wants VariableDeclaration
            const declarations = decl.declarations || [decl];
            const vids = declarations.flatMap(({ id }) =>
              collectPatternIdentifiers(path, id),
            );
          ...
          for (const spec of specs) {
            // @ts-expect-error - wants ExportSpecifier
