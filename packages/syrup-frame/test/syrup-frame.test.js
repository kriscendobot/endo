...
test('reads message at exactly maxMessageLength', async t => {
  ...
  const r = makeSyrupsReader([encoder.encode('10:'), payload], {
    maxMessageLength: 10,
  });
  const got = await read(r);
...
test('reads message with leading-zero prefix', async t => {
  await null;
  const r = makeSyrupsReader([encoder.encode('05:hello')]);
  const got = await read(r);
...
test('two adjacent frames in one chunk read as two', async t => {
  await null;
  const r = makeSyrupsReader([encoder.encode('5:hello5:world')]);
  t.deepEqual(await read(r), [
    encoder.encode('hello'),
...
test('maxMessageLength: 0 accepts empty frame and rejects non-empty', async t => {
  await null;
  const accepting = makeSyrupsReader([encoder.encode('0:')], {
    maxMessageLength: 0,
  });
  t.deepEqual(await read(accepting), [encoder.encode('')]);

  const rejecting = makeSyrupsReader([encoder.encode('1:A')], {
    maxMessageLength: 0,
  });
...
// by the syrups contract; documented here as the explicit invariant.
test('partial prefix at EOF is silently discarded (matches netstring)', async t => {
  await null;
  const r = makeSyrupsReader([encoder.encode('123')]);
  t.deepEqual(await read(r), []);
});
